...
encryptFile()
    - Encrypts the file by reading its info,
    changing to encyrpted text for each line,
    and saving the result to a new file.
    - parameters needed: None.
    - return: None.

decryptFile()
    - Decrypts a file by reading its contents,
    applying decryption to each line,
    and saving the result to a new file.
    - parameters needed: None.
    - return: None.
...
updateMode(newMode: str)
    ...
    - parameters needed: newMode (str) - the new mode to set ("auto" or "manual").
    - return: None.
